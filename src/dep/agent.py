...
def get_stock_price_from_yfinance(ticker: str) -> dict:
    ...
    stock_info = yf.download(ticker, start=start.strftime("%Y-%m-%d"), end=end.strftime("%Y-%m-%d")).to_dict()
    return stock_info
...
class LiquidityAgent:
    ...
    If the trend of 6-months is up, then the liquidity is good.
    If the trend of 6-months is down, then the liquidity is bad.
    """
...
system_prompt = (
    ...
    " For liquidity_check, always use liquidity_check agent and do not invent your own logic."
